...
MANNHEIM_CONFIG = {
    ...
    'districts': [
        ...
        'Käfertal', 'Waldhof', 'Sandhofen', 'Rheinau'
    ],
    'key_areas': {
        'industrial': 'Industriehafen, BASF-Nähe',
...
class MannheimDataCollector:
    """Spezifischer Data Collector für Mannheim"""
    ...
    def get_mannheim_overpass_query(self, feature_type):
        """Erstellt Overpass-Query für Mannheim"""
    ...
    def simulate_mannheim_temperature_grid(self, grid_size=50):
        """Simuliert Temperaturdaten für Mannheim basierend auf realen Gegebenheiten"""
        np.random.seed(42)
        ...
        for lat in lat_range:
            for lon in lon_range:
                ...
                rhein_dist = abs(lon - 8.4400)  # Nähe zum Rhein
                neckar_dist = abs(lat - 49.4900)  # Nähe zum Neckar

                # Basis-Temperatur je nach Lage
                base_temp = 24.0

                # Innenstadt-Effekt (höhere Temperaturen)
                ...
                # Industriegebiet (BASF-Nähe, Hafen)
...
class MannheimHeatAnalyzer:
    """Heat Island Analyse speziell für Mannheim"""
    ...
    def __init__(self):
        self.mannheim_factors = {
            ...
            'distance_to_water': 0.15,  # Rhein/Neckar Kühlungseffekt
            'industrial_proximity': 0.30  # BASF/Hafen Wärmeeffekt
        }
        ...
        for factor, weight in self.mannheim_factors.items():
            if factor in df.columns:
                if factor == 'distance_to_water':
                    # Umkehrung: größere Distanz = höherer Heat Index
...
# Demo für Mannheim
print("🏭 MANNHEIM URBAN HEAT EXPLORER")
print("=" * 50)
...
print(f"📊 Datensatz erstellt: {len(mannheim_data)} Messpunkte")
print(f"🌡️  Temperaturbereich: {mannheim_data['temperature'].min():.1f}°C - {mannheim_data['temperature'].max():.1f}°C")
...
for area_type in mannheim_analyzed['area_type'].unique():
    ...
    print(f"  {area_type.upper()}: Ø {avg_temp:.1f}°C, {hotspot_count}/{total_count} Hotspots")

# Hotspot-Analyse
hotspots = mannheim_analyzed[mannheim_analyzed['is_hotspot']]
print(f"\n🔥 HEAT ISLAND HOTSPOTS ({len(hotspots)} Gebiete):")
...
print(f"\n❄️  KÜHLSTE GEBIETE ({len(cool_spots)} Gebiete):")
print(f"   Durchschnittstemperatur: {cool_spots['temperature'].mean():.1f}°C")
print(f"   Hauptsächlich: {cool_spots['area_type'].mode().iloc[0]}")

print(f"\n📍 MANNHEIM-SPEZIFISCHE ERKENNTNISSE:")
print(f"   • Industriegebiet (BASF-Nähe): Höchste Temperaturen")
print(f"   • Rhein/Neckar-Nähe: Kühlungseffekt erkennbar")
print(f"   • Luisenpark-Bereich: Deutliche Temperaturreduktion")
print(f"   • Innenstadt: Typischer Urban Heat Island Effekt")

# Nächste Schritte für Mannheim
print(f"\n🎯 NÄCHSTE SCHRITTE FÜR MANNHEIM:")
print("   1. Echte Wetterstationen-Daten von DWD integrieren")
print("   2. OSM-Daten für Mannheim abrufen")
print("   3. Satellitendaten (Landsat/Sentinel) für LST")
